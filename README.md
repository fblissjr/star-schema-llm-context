...
While it was always stuck in my head, I took detours into interpretability and SAEs given the somewhat orthogonal ties to what I was trying to do. Then came (Facebook's Large Concept Model)[https://github.com/facebookresearch/large_concept_model], and while it was aimed mostly at semantic definitions over dense embeddings (a dead end when it comes to scaling), it reinforced and built new areas for me to look into - not the idea of semantic embeddings, but the idea of semantic abstractions within the inference process (or as close to it as we can get). (RLMs, or Recursive Language Models)[https://arxiv.org/abs/2512.24601v1], is an active research area for me here - primarily because of its simplicity and potential as a verifier (or routing?) model.
